...
The theorem states that any distributed store can achieve both consistency and availability in healthy network conditions, but can only guarantee one of the two in the case of a network partition, and all distributed systems must account for network partitions. There are two main types of systems which prioritize certain features: AP (availability + partition tolerance) and CA (consistency + availability). Using a multi-node distributed cache as an example, we can look at various implementation details which differentiate each type of system.
